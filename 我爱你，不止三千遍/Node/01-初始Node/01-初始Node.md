...
答：每个浏览器都内置了 DOM、BOM 相关的 API 接口函数，因此浏览器中的 JavaScript 才能调用它们。
...
Node.js 是 JS 语言的独立运行时，而 JS 是一门单线程语言，但它利用 “事件循环+任务队列” 的方式实现了 “单线程下的异步编程”，且该异步具有 “非阻塞” 的特点。简而言之，Node.js 开发的系统具有 “单线程异步非阻塞” 的重大优势！相较于 “多线程异步” 来说，计算机不用考虑多个线程之间的切换和同步等问题，所以能利用更少的硬件资源处理更多的任务，同时 “非阻塞” 的特点还能避免 “多线程异步” 经常遇到的线程阻塞问题。所以，Node.js 开发的系统具有天生的 “IO高并发” 优势！

值得注意的是：Node.js 的 “单线程异步非阻塞” 也不是万能的，它比较适合 “IO密集型” 的项目，例如：API 接口项目（大部分时间都是在处理网络IO/文件IO/数据库IO等）。但是对于 “CPU密集型” 的项目，Node.js 就不行了，例如：涉及大量的图像处理、视频转码、加密解密等任务的项目，原因是 JS 是单线程的，它只能调用单个线程来工作，而不能充分利用 CPU 空闲的资源，也无法利用 GPU 资源，所以对于计算密集的项目，Node.js 就 “吃不消”！相反的，像 C/C++、Java 这类多线程语言来说，CPU 密集型任务却是他们比较擅长的地方！当然，也不是说只要遇到计算型任务就不能用 Node.js 来解决，只要计算任务不是特别的密集，计算量也不是特别重，那么还是可以用 Node.js 来处理的，因为 Node.js 可以通过创建子进程的方式去调用 CPU 的空闲能力，可以把 Node.js 中的 CPU 计算密集型任务的部分单独开子进程来进行处理！（可以借助 Node.js 专门的多进程管理工具 PM2），或者直接在一台服务器上部署多个 Node.js 实例（可以借助 Docker），一些实例负责 IO，一些实例负责 CPU 计算，同时利用负载均衡管理工具来实现请求的均匀分发（可以借助 Nginx），那么也能解决大部分的 CPU 计算密集型问题！当然，在实际项目中，单纯用 Node.js 来解决 IO+CPU 的情况是比较少的，一般遇到涉及一定 CPU 密集任务的项目，通常 IO 由 Node.js 来负责，而 CPU 计算用另外的语言（比如：C/C++）来负责更为妥当！比如：网络请求过来后先进入 Node.js，然后遇到计算任务的部分，Node.js 再调用 C/C++ 编写的计算接口去处理，然后当计算结果出来后再返回给 Node.js，然后 Node.js 再返回给客户端。

> 注意：JS 是单线程的！但是 Node.js 不是单线程的！Node.js 是 JS 的独立运行环境，与 JS 不是一个层面的东西！许多人之所以误解 Node.js 是单线程的，那是因为我们基于 Node.js 编写的 JS 代码是单线程运行的！导致许多人误以为 Node.js 也是单线程的！这里一定要注意！
