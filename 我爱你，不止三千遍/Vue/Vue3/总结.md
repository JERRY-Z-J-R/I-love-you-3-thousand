...
- 由于是组合式 API，我们可以直接把计算属性追加到对象中，例如有个 Student 对象，里面有 firstName 和 lastName 两个属性，那么按理来说 fullName 应该也属于 Student 对象的属性，所以我们可以直接把计算属性加到 Student 对象里，`Student.fullName = computed(...)` ！并且由于 Student 是 reactive 实现的，所以我们根本不用担心新增属性的响应式！（这也是 Vue3 的优点之一）
