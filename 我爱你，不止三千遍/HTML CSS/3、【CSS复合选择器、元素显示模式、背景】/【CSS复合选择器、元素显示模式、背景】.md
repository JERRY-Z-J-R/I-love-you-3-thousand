...
`伪类选择器` 用于**向某些选择器添加特殊的效果**，比如：给链接添加特殊效果（链接伪类），或选择第 1 个，第 n 个元素（结构伪类）。
...
常见的块元素有 `<h1> ~ <h6>`、`<p>`、`<div>`、`<ul>`、`<ol>`、`<li>`、`<dl>`、`dt`、`dd`、`<table>`、`<form>` 等，其中  `<div>` 标签是最典型的块元素。
...
常见的行内元素有 `<a>`、`<span>`、`<em>` 等，其中 `<span>` 标签是最典型的行内元素，有的地方也将行内元素称为内联元素。
